"""``transform_spec`` owns and releases the LookupIndex it creates.

These exercise ``engine.py``'s ``owns_index`` bookkeeping — that a caller-supplied
index is left alone, and an index the engine created is closed and detached on
completion, on exception, and on early iterator close.

Split out of ``test_utils/test_lookup_index_context_manager.py`` (issue #298),
where they read as ``LookupIndex`` tests despite exercising the engine.

Originally contributed in PR #144. See: https://github.com/linkml/linkml-map/issues/143
"""
