# Auto generated from agent.yaml by pythongen.py version: 0.0.1
# Generation date: 2026-05-18T10:41:00
# Schema: personinfo
#
...
class Entity(YAMLRoot):
    """
    Abstract base for derived target classes that live in the agent world, independent of any class hierarchy in the
    source schema.
    """
    _inherited_slots: ClassVar[list[str]] = []

    class_class_uri: ClassVar[URIRef] = PERSONINFO["Entity"]
    class_class_curie: ClassVar[str] = "personinfo:Entity"
    class_name: ClassVar[str] = "Entity"
    class_model_uri: ClassVar[URIRef] = PERSONINFO.Entity
...
class FamilialRelationshipType(EnumDefinitionImpl):
    ...
    SIBLING_OF = PermissibleValue(
        text="SIBLING_OF",
        meaning=FAMREL["01"])
    PARENT_OF = PermissibleValue(
        text="PARENT_OF",
        meaning=FAMREL["02"])
    CHILD_OF = PermissibleValue(
        text="CHILD_OF",
        meaning=FAMREL["05"])
...
slots.label = Slot(uri=SCHEMA.name, name="label", curie=SCHEMA.curie('name'),
                   model_uri=PERSONINFO.label, domain=None, range=Optional[str])
...
slots.secondary_email = Slot(uri=SCHEMA.email, name="secondary_email", curie=SCHEMA.curie('email'),
                   model_uri=PERSONINFO.secondary_email, domain=None, range=Optional[str])

slots.birth_date = Slot(uri=SCHEMA.birthDate, name="birth_date", curie=SCHEMA.curie('birthDate'),
                   model_uri=PERSONINFO.birth_date, domain=None, range=Optional[Union[str, XSDDate]])

slots.death_date = Slot(uri=PERSONINFO.death_date, name="death_date", curie=PERSONINFO.curie('death_date'),
                   model_uri=PERSONINFO.death_date, domain=None, range=Optional[Union[str, XSDDate]])
...
slots.driving_since = Slot(uri=PERSONINFO.driving_since, name="driving_since", curie=PERSONINFO.curie('driving_since'),
                   model_uri=PERSONINFO.driving_since, domain=None, range=Optional[Union[str, XSDDate]])

slots.first_known_event = Slot(uri=PERSONINFO.first_known_event, name="first_known_event", curie=PERSONINFO.curie('first_known_event'),
                   model_uri=PERSONINFO.first_known_event, domain=None, range=Optional[Union[str, XSDDate]])

slots.parents = Slot(uri=PERSONINFO.parents, name="parents", curie=PERSONINFO.curie('parents'),
                   model_uri=PERSONINFO.parents, domain=None, range=Optional[Union[str, list[str]]])
...
slots.agents = Slot(uri=PERSONINFO.agents, name="agents", curie=PERSONINFO.curie('agents'),
                   model_uri=PERSONINFO.agents, domain=None, range=Optional[Union[dict[Union[str, AgentId], Union[dict, Agent]], list[Union[dict, Agent]]]])
...
slots.Agent_primary_email = Slot(uri=SCHEMA.email, name="Agent_primary_email", curie=SCHEMA.curie('email'),
                   model_uri=PERSONINFO.Agent_primary_email, domain=Agent, range=Optional[str],
                   pattern=re.compile(r'^\S+@[\S+\.]+\S+'))

slots.Agent_secondary_email = Slot(uri=SCHEMA.email, name="Agent_secondary_email", curie=SCHEMA.curie('email'),
                   model_uri=PERSONINFO.Agent_secondary_email, domain=Agent, range=Optional[str],
                   pattern=re.compile(r'^\S+@[\S+\.]+\S+'))
...
slots.FamilialRelationship_related_to = Slot(uri=PERSONINFO.related_to, name="FamilialRelationship_related_to", curie=PERSONINFO.curie('related_to'),
                   model_uri=PERSONINFO.FamilialRelationship_related_to, domain=FamilialRelationship, range=Union[str, AgentId])
