...
def join_keys(spec: AliasedClass) -> tuple[str, str]:
    ...
    ``join_on`` is shorthand for when both sides share a column name; an explicit
    ``source_key``/``lookup_key`` pair overrides it. This is the single source of
    truth for that precedence — every join path (the per-row lookup, the set-based
    engine, and the normalizer's validation) resolves keys through here so they can
    never drift.

    :param spec: The join specification.
    :returns: ``(source_key, lookup_key)`` — the primary-side and joined-side columns.
    :raises ValueError: If neither ``join_on`` nor both explicit keys are set.
    """
