...
class SpecificationComponent(ConfiguredBaseModel):
    description: Optional[str] = Field(None, description="""description of the specification component""")
    implements: Optional[List[str]] = Field(default_factory=list, description="""A reference to a specification that this component implements.""")
    comments: Optional[List[str]] = Field(default_factory=list, description="""A list of comments about this component. Comments are free text, and may be used to provide additional information about the component, including instructions for its use.""")
...
class TransformationSpecification(SpecificationComponent):
    ...
    """
    id: Optional[str] = Field(None, description="""Unique identifier for this transformation specification""")
    title: Optional[str] = Field(None, description="""human readable title for this transformation specification""")
    prefixes: Optional[Dict[str, KeyVal]] = Field(default_factory=dict, description="""maps prefixes to URL expansions""")
    source_schema: Optional[str] = Field(None, description="""name of the schema that describes the source (input) objects""")
    target_schema: Optional[str] = Field(None, description="""name of the schema that describes the target (output) objects""")
    class_derivations: Optional[Dict[str, ClassDerivation]] = Field(default_factory=dict, description="""Instructions on how to derive a set of classes in the target schema from classes in the source schema.""")
    enum_derivations: Optional[Dict[str, EnumDerivation]] = Field(default_factory=dict, description="""Instructions on how to derive a set of enums in the target schema""")
    slot_derivations: Optional[Dict[str, SlotDerivation]] = Field(default_factory=dict, description="""Instructions on how to derive a set of top level slots in the target schema""")
    description: Optional[str] = Field(None, description="""description of the specification component""")
    implements: Optional[List[str]] = Field(default_factory=list, description="""A reference to a specification that this component implements.""")
    comments: Optional[List[str]] = Field(default_factory=list, description="""A list of comments about this component. Comments are free text, and may be used to provide additional information about the component, including instructions for its use.""")
...
class ElementDerivation(SpecificationComponent):
    ...
    value_mappings: Optional[Dict[str, KeyVal]] = Field(default_factory=dict, description="""A mapping table that is applied directly to mappings, in order of precedence""")
    expression_to_value_mappings: Optional[Dict[str, KeyVal]] = Field(default_factory=dict, description="""A mapping table in which the keys are expressions""")
    expression_to_expression_mappings: Optional[Dict[str, KeyVal]] = Field(default_factory=dict, description="""A mapping table in which the keys and values are expressions""")
...
class ClassDerivation(ElementDerivation):
    ...
    """
    populated_from: Optional[str] = Field(None, description="""Name of the class in the source schema""")
    sources: Optional[List[str]] = Field(default_factory=list)
    joins: Optional[Dict[str, AliasedClass]] = Field(default_factory=dict, description="""Additional classes to be joined to derive instances of the target class""")
...
class SlotDerivation(ElementDerivation):
    ...
    sources: Optional[List[str]] = Field(default_factory=list)
    derived_from: Optional[List[str]] = Field(default_factory=list, description="""Source slots that are used to derive this slot. This can be computed from the expr, if the expr is declarative.""")
    expr: Optional[str] = Field(None, description="""An expression to be evaluated on the source object to derive the target slot. Should be specified using the LinkML expression language.""")
...
class EnumDerivation(ElementDerivation):
    ...
    expr: Optional[str] = Field(None, description="""An expression to be evaluated on the source object to derive the target slot. Should be specified using the LinkML expression language.""")
    hide: Optional[bool] = Field(None, description="""True if this is suppressed""")
    permissible_value_derivations: Optional[Dict[str, PermissibleValueDerivation]] = Field(default_factory=dict, description="""Instructions on how to derive a set of PVs in the target schema""")
    copy_directives: Optional[Dict[str, CopyDirective]] = Field(default_factory=dict)
    overrides: Optional[Any] = Field(None, description="""overrides source schema slots""")
    is_a: Optional[str] = Field(None)
    mixins: Optional[List[str]] = Field(default_factory=list)
